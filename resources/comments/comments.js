...
(function () {
  ...
  async function sendComment(scope, parentId) {
    ...
    var attachments = (scope === 'top' ? state.composeAtt : (composeEl._attachments || [])).slice(0, MAX_ATTACH);
  ...
  async function handleFileInput(inp, composeEl) {
    ...
    inp.value = '';
    var arr = composeEl._attachments = composeEl._attachments || (composeEl.dataset.scope === 'top' ? state.composeAtt : []);
  ...
  function renderAttachments2(composeEl) {
    var attBox = composeEl.querySelector('.cm-compose-attachments');
    if (!attBox) return;
    var arr = composeEl._attachments || (composeEl.dataset.scope === 'top' ? state.composeAtt : []);
    var h = '';
