(function() {
      ...
      return new Promise(function(resolve, reject) {
        ...
        promises.forEach(function(p, i) {
          Promise.resolve(p).then(resolve, function(err) {
            ...
            pending--;
            if (pending === 0) {
              reject(new Error('All promises were rejected: ' + errors.map(function(e) { return e && e.message; }).join(', ')));
            }
          });
        });
      });
  ...
  // ========== 加载某一分区的所有数据（用于分页计算和全站搜索） ==========
    ...
    if (manifest && manifest[dir]) {
      ...
      var allData = dataArrays.reduce(function(a, b) { return a.concat(b); }, []);
