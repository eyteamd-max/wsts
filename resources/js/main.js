(function() {
  ...
  async function loadAllDataForCategory(categoryKey) {
    ...
    if (manifest && manifest[dir]) {
      ...
      for (const idx of indices) {
        const data = await loadJsonByManifest(categoryKey, idx);
  ...
  async function openModal(mod) {
    ...
    let cancelled = false;
    const cleanupLoading = function() {
      if (loadingOverlayEl.parentNode) loadingOverlayEl.remove();
      if (closeBtn.parentNode) closeBtn.remove();
    };
    closeBtn.addEventListener('click', function() { cancelled = true; cleanupLoading(); currentMod = null; });
    ...
    try {
      const results = await Promise.all(
        imageCandidates.map(function(item) {
          return raceImageWithRetry(toCandidates(item), 2).catch(function() { return null; });
        })
      );
      if (cancelled) return;
      finalImages = results.filter(function(url) { return url !== null; });
  ...
  async function performGlobalRidSearch(rid) {
    ...
    for (const cat of categories) {
      ...
      if (allSiteData[cat]) {
        allData.push.apply(allData, allSiteData[cat]);
      }
    }
    for (let i = 0; i < allData.length; i++) {
      if (allData[i].id === rid) return allData[i];
    }
    return null;
  }
