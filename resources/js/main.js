(function() {
  ...
  const dataSources = {
    all: 'resources/json/post/sts2_mods/sts2_mods.json',
    skin: 'resources/json/post/O.o_interface/O.o_interface.json'
  };
  const dataCache = {};
  ...
  function updateCarousel(images) {
    ...
    carouselTrack.innerHTML = currentImages.map((src, idx) =>
      `<div class="carousel-slide" data-index="${idx}"><img src="${src}" alt="预览图 ${idx+1}"></div>`
    ).join('');
    ...
    carouselTrack.querySelectorAll('.carousel-slide').forEach(slide => {
      slide.addEventListener('click', () => {
        ...
        if (currentImages[idx]) { lightboxImg.src = currentImages[idx]; lightboxOverlay.classList.add('active'); }
      });
    });
    carouselDots.querySelectorAll('.carousel-dot').forEach(dot => {
      dot.addEventListener('click', (e) => { setCurrentIndex(parseInt(e.target.dataset.index)); });
    });
