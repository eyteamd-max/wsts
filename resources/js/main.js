(function() {
    ...
    const dataSources = { all: 'resources/json/sts2_mods.json', skin: 'resources/json/O.o_interface.json' };
    const dataCache = {};
    ...
    function renderModCards(dataArray) {
        ...
        const loaded2Src = window.loaded2GifSrc || 'resources/img/loaded_2.gif';
    ...
    async function initPage() {
        let loadingGifUrls = ['resources/img/loaded.gif'];
        let logoUrls = ['resources/img/Lihui.gif'];
        let loaded2GifUrls = [
            'resources/img/loaded_2.gif',
        ...
        setTimeout(() => {
            ...
            loadModData('all'); // 开始加载MOD数据
        }, 100);

        // 后台更新图片
        gifPromise.then(src => {
            if (src) {
                // 如果加载到GIF，可替换掉CSS动画（如果需要）
                loadingGif.src = src;
