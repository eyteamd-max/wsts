(function() {
  ...
  // ========== 详情页打开（去掉轮播框） ==========
    ...
    let coverCandidates = [];
    if (mod.coverImage) {
      if (Array.isArray(mod.coverImage)) coverCandidates = mod.coverImage.filter(function(u){ return u && u.trim(); });
      else if (typeof mod.coverImage === 'string' && mod.coverImage.trim()) coverCandidates = [mod.coverImage];
    }
    if (coverCandidates.length > 0) {
      raceImageWithRetry(coverCandidates, 1).then(function(url) {
        const img = document.createElement('img');
        img.src = url;
        img.alt = mod.title;
        img.addEventListener('click', function() { openLightbox(url); });
