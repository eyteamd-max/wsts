  ...
  function raceVideo(urls) {
    ...
    var promises = urls.map(function(url) {
      return new Promise(function(resolve, reject) {
        ...
        var timer = setTimeout(function() { reject(new Error('video load stalled')); }, 8000);
        video.onloadeddata = function() { clearTimeout(timer); resolve({ url: url, video: video }); };
        video.onerror = function() { clearTimeout(timer); reject(new Error('video load failed')); };
        video.src = url;
        videos.push(video);
      });
    });
    return Promise.any(promises).then(function(result) {
      ...
      videos.forEach(function(v) { v.onloadeddata = v.onerror = null; v.src = ''; });
      throw e;
    });
  }
...
function attachCardSpinner(cardElement) {
  ...
  setTimeout(function() { if (coverImg.complete) hideSpinner(); }, 500);
...
function renderModCards(dataArray) {
  ...
  async function initPage() {
    ...
    // ===== 关键：分层优先级预加载 =====
    // RID 参数优先：如果 URL 带 rid，先做最小必要的数据加载，
    // 不让预加载阻塞 RID 跳转流程
    var urlParams = new URLSearchParams(window.location.search);
    var hasRid = !!urlParams.get('rid');

    // 启动分层预加载（带 6 秒上限保护，绝不卡死）
    ...
    if (allSiteData['all']) {
      ...
      // 兜底：万一预加载彻底失败，正常走 loadModData
      loadModData('all');
    }

    // 处理 RID 跳转（在主界面渲染后立即触发，无缝衔接弹窗）
    if (hasRid) {
      handleUrlParams();
    }
